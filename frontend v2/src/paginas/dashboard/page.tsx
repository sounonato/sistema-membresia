...
export function DashboardPage() {
  ...
  const stats = data ?? {};
  const porMes = (stats.convertidos_por_mes ?? stats.por_mes ?? []).map((r: { mes: string; quantidade: number }) => ({
  ...
  return (
    <div className="space-y-16 text-stone-900">
      ...
      <header className="border-b border-stone-300 pb-8">
        <div className="flex items-center justify-between text-xs tracking-widest uppercase text-stone-500 mb-5">
          <span>Panorama pastoral</span>
          <span className="font-editorial italic normal-case tracking-normal text-stone-500 text-sm">
            {dateLabel}
          </span>
        </div>
        <h1 className="font-serif text-4xl sm:text-5xl leading-tight tracking-tight font-light">
          Bom te ver,{" "}
          <span className="font-editorial italic text-amber-800">{usuario?.nome?.split(" ")[0] ?? "irmão"}</span>.
        </h1>
        <p className="mt-4 max-w-xl text-sm text-stone-600 leading-relaxed">
      ...
      {/* KPIs */}
      <section>
        <p className="text-xs tracking-widest uppercase text-stone-500 mb-5 flex items-center gap-3">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 border-t border-stone-900">
          {kpis.map((k, i) => (
            <div
              key={k.label}
              className={
                "py-6 pr-4 border-b border-stone-300 " +
                (i < 3 ? "lg:border-r lg:border-stone-200 lg:pr-8 " : "") +
                (i % 2 === 0 ? "border-r border-stone-200 pr-6 lg:pr-8" : "")
              }
            >
              <p className="font-editorial italic text-amber-700 text-xs mb-2">{k.n}</p>
              <p className="font-serif text-5xl leading-none tabular-nums text-stone-900 font-light">
                {k.value}
              </p>
              <p className="mt-3 text-sm text-stone-900 font-medium">{k.label}</p>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={porMes} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                ...
                />
                <Bar dataKey="quantidade" fill="#b45309" radius={[0, 0, 0, 0]} maxBarSize={48} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <ul className="mt-4 space-y-2">
            {porGenero.map((g, i) => (
              ...
              >
                ...
                <span className="font-serif tabular-nums text-lg">{g.quantidade}</span>
              </li>
            ))}
          </ul>
        </div>
