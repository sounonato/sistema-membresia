...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((u: Usuario) => {
        if (!alive) return;
        setUsuario(u);
        const slugFromMe = u.igreja?.slug ?? u.igreja_slug ?? null;
        if (slugFromMe) {
          setSlug(slugFromMe);
          localStorage.setItem("slug", slugFromMe);
        }
      })
      .catch(() => {
        if (!alive) return;
  ...
  async function login(email: string, senha: string, slugIgreja: string) {
    ...
    const slugReal = res.usuario?.igreja_slug ?? slugIgreja ?? "";
    localStorage.setItem("token", t);
    localStorage.setItem("slug", slugReal);
    setToken(t);
    setSlug(slugReal);
