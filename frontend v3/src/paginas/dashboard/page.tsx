...
import { useDashboardStats, useMembrosStatsDashboard } from "./hooks";
...
export function DashboardPage() {
  ...
  const { data: membrosStats } = useMembrosStatsDashboard();
  ...
  return (
    <div className="space-y-16 text-stone-900">
      ...
      {membrosStats && (
        <section>
          <p className="text-[10px] tracking-[0.3em] uppercase text-stone-500 mb-6 flex items-center gap-3">
            <span className="tabular-nums">II.</span>
            <span className="h-px w-8 bg-stone-400" />
            Membresia formal
          </p>

          {/* KPIs de membros */}
          <div className="grid grid-cols-2 lg:grid-cols-4 border-t border-stone-900 mb-10">
            {[
              { n: "01", label: "Membros", value: membrosStats.total, note: "Total registrado" },
              { n: "02", label: "Ativos", value: membrosStats.ativos, note: "Em comunhão" },
              { n: "03", label: "Sem contato", value: membrosStats.sem_contato_60, note: "Há mais de 60 dias" },
              { n: "04", label: "Curso de Membresia", value: membrosStats.fez_curso_membresia, note: "Formados (Nazareno)" },
            ].map((k, i) => (
              <div
                key={k.label}
                className={
              ...
              >
                <p className="font-editorial italic text-amber-800 text-sm mb-3">{k.n}</p>
                <p className={`font-serif text-5xl leading-none tabular-nums font-light ${i === 2 && k.value > 0 ? "text-red-700" : "text-stone-900"}`}>
                  {k.value}
                </p>
                <p className="mt-4 text-sm text-stone-900 font-medium">{k.label}</p>
                <p className="text-xs text-stone-500 mt-1">{k.note}</p>
          ...
          <div className="border border-stone-200 p-6">
            <p className="text-[10px] tracking-[0.3em] uppercase text-stone-500 mb-4">Distribuição por status</p>
            {(() => {
              const total = membrosStats.ativos + membrosStats.inativos + membrosStats.transferidos || 1;
              const bars = [
                { label: "Ativos", value: membrosStats.ativos, color: "#b45309" },
                { label: "Inativos", value: membrosStats.inativos, color: "#a8a29e" },
                { label: "Transferidos", value: membrosStats.transferidos, color: "#78716c" },
              ];
              return (
                <>
                  ...
                  <div className="flex gap-6 mt-3">
                    {bars.map((b) => (
                      <span key={b.label} className="flex items-center gap-2 text-xs text-stone-600">
                        <span className="h-2 w-2 rounded-full" style={{ background: b.color }} />
                        {b.label}: <strong className="font-serif">{b.value}</strong>
                      </span>
                    ))}
                  </div>
                </>
      ...
      <section className="grid lg:grid-cols-12 gap-x-8 gap-y-12">
          ...
          <div className="flex items-baseline justify-between mb-6 border-b border-stone-300 pb-4">
            <div>
              <p className="text-[10px] tracking-[0.3em] uppercase text-stone-500 mb-1">
                <span className="tabular-nums">III.</span> — Colheita mensal
              </p>
        ...
        <div className="lg:col-span-4">
          <div className="mb-6 border-b border-stone-300 pb-4">
            <p className="text-[10px] tracking-[0.3em] uppercase text-stone-500 mb-1">
              <span className="tabular-nums">IV.</span> — Retrato
            </p>
