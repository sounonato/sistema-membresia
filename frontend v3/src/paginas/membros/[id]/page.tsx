            ...
            <TabsContent value="eclesiastico" className="mt-4">
              <Card>
                <dl className="space-y-2 text-sm">
                  ...
                  <Info label="Curso de Membresia (Nazareno)">{m.fez_curso_membresia ? "Sim" : "Não"}</Info>
                  {m.data_conversao && (
                    <Info label="Data de conversão (aceitou a Jesus)">{formatDate(m.data_conversao)}</Info>
                  )}
