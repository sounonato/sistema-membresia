...
import {
  useCreateDiscipulador,
  useDeleteDiscipulador,
  useDiscipuladores,
  useUpdateDiscipulador,
  type Discipulador,
} from "./hooks";
...
export function DiscipuladoresPage() {
  ...
  function abrirEdit(d: Discipulador) {
    setEdit(d);
    setForm(d);
    setOpen(true);
  }
  ...
  return (
    <div>
        ...
        actions={
          editor && (
            <Button
              ...
              onClick={abrirNovo}
            >
              <Plus className="h-4 w-4" /> Novo(a) discipulador(a)
            </Button>
          )
        }
      />
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-border">
                    ...
                    <TableHead className="text-[10px] uppercase tracking-widest text-muted-foreground text-right">
                      Discipulado
                    </TableHead>
                    <TableHead className="text-[10px] uppercase tracking-widest text-muted-foreground">
                      Acesso
                ...
                <TableBody>
                  {data.map((d, i) => (
                    <TableRow key={d.id} className="border-border hover:bg-muted">
                      ...
                      <TableCell className="text-right font-serif text-lg tabular-nums text-foreground">
                        {d.qtd_grupos ?? 0}
                      </TableCell>
                      <TableCell>
                        {d.usuario_email ? (
                      ...
                      {editor && (
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-1">
                            <Button size="icon" variant="ghost" onClick={() => abrirEdit(d)}>
        ...
        <DialogContent className="rounded-2xl">
          <DialogHeader>
            <DialogTitle className="font-serif text-primary">
              {edit ? "Editar Discipulador(a)" : "Novo(a) Discipulador(a)"}
            </DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={acessoOpen} onOpenChange={setAcessoOpen}>
        <DialogContent className="rounded-2xl">
          <DialogHeader>
            <DialogTitle className="font-serif text-primary">Criar Acesso ao Sistema</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div className="text-sm text-muted-foreground">
              Crie uma conta de login para o(a) discipulador(a){" "}
              <strong>{acessoDiscipulador?.nome}</strong>.
            </div>
            <Campo label="E-mail">
              <Input
                type="email"
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAcessoOpen(false)}>
              Cancelar
            </Button>
