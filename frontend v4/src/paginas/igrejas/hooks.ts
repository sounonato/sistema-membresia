...
import { api, type Igreja, type SolicitacaoIgreja } from "@/lib/api";
...
export function useSolicitacoes(status?: string) {
  return useQuery<SolicitacaoIgreja[]>({
    queryKey: ["solicitacoes", status],
    queryFn: () => api.getSolicitacoes(status),
  });
}
...
export function useAprovarSolicitacao() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.aprovarSolicitacao(id),
    onSuccess: () => {
...
export function useRejeitarSolicitacao() {
  ...
  return useMutation({
    mutationFn: ({ id, motivo }: { id: string; motivo?: string }) =>
      api.rejeitarSolicitacao(id, motivo),
