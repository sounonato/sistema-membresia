...
function QrCard({
  ...
  const titulo = tipo === "convertido" ? (grupoNome ?? "Faça parte") : "Seja membro";
  ...
  return (
    <div className="bg-card border border-border flex flex-col">
      ...
      <div className="flex flex-col items-center justify-center text-center p-8 flex-1">
        ...
        <p className="font-[Instrument_Serif,serif] italic text-base text-muted-foreground mt-1">
          {subtitulo}
        </p>
...
export function QrCadastroPage() {
  ...
  const grupoNome = grupoId !== "__geral" ? grupos?.find((g) => g.id === grupoId)?.nome : null;

  if (!slugAtivo) {
    return (
      <div className="text-muted-foreground italic font-serif">
        Nenhuma igreja vinculada à sua conta.
      </div>
    );
  }
