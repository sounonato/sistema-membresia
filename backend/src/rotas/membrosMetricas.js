...
router.get('/membros/metricas', checkPerfil(['admin', 'lider', 'pastor']), async (req, res) => {
  try {
    ...
    const faixaEtariaRes = await db.query(
      ...
      ORDER BY 
        CASE 
          WHEN faixa = '0-17' THEN 1 
          WHEN faixa = '18-24' THEN 2 
          WHEN faixa = '25-34' THEN 3 
          WHEN faixa = '35-44' THEN 4 
          WHEN faixa = '45-54' THEN 5 
          WHEN faixa = '55-64' THEN 6 
          WHEN faixa = '65+' THEN 7 
          ELSE 8 
        END`,
      [igrejaId]
    );
